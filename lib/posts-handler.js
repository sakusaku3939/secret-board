    ...
    switch (req.method) {
        case 'GET':
            ...
            Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
                posts.forEach((post) => {
                    post.name = post.name == null ? '名無しさん' : post.name.replace(/\+/g, ' ');
        ...
        case 'POST':
            ...
            req.on('data', (chunk) => {
                ...
                const matchResult = decoded.match(/name=([\s\S]*)&content=([\s\S]*)&oneTimeToken=(.*)/);
                if (!matchResult) {
                    ...
                    if (oneTimeTokenMap.get(originalTrackingId) === requestedOneTimeToken) {
                        if (name !== "" && content !== "") {
                            console.info('投稿されました: name: ' + name + ', content: ' + content);
                            Post.create({
                                name: name,
                                content: content,
...
function handleLogin(req, res) {
    ...
    switch (req.method) {
        case 'GET':
            ...
            const oneTimeToken = crypto.randomBytes(8).toString('hex');
            oneTimeTokenMap.set(originalTrackingId, oneTimeToken);
            res.end(pug.renderFile('./views/login.pug', {
                oneTimeToken: oneTimeToken,
            ...
            req.on('data', (chunk) => {
                ...
                if (!matchResult) {
                    ...
                    if (oneTimeTokenMap.get(originalTrackingId) === requestedOneTimeToken) {
                        ...
                        // TODO ログインに失敗した際の処理追加
                        if (result) {
                            messageType = 1;
                            handleRedirectPosts(req, res);
                        } else {
                            isLoginFailed = true;
                            handleRedirectLogin(req, res);
                        }
                    } else {
                        util.handleBadRequest(req, res);
                    }
...
function usersData() {
    const fileData = fs.readFileSync("./users.userdata", "utf8");
